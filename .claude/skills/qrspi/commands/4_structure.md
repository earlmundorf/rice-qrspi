...
1. Break the work into **vertical slices** — each slice delivers one testable capability
   ...
   like in this repo's stack; follow its shape (when the field is absent, infer the shape
   from the stack). Never horizontal layers ("all types, then all components").
